...
​ 任务（Task）描述数栖开发平台数据分格和处理过程的基本单元。泛指对数据执行的操作，通常指的是节点任务，也就是单个任务。具体任务类型请详见任务类型模块(4.1.3)。任务可自由配置参数、调度时间、任务间的依赖关系和任务基线等。多个任务间可配置依赖关系，组成DAG图。

- 脚本

​  脚本(script)主要给数据研发人员调试使用，仅能进行查询和分析数据操作，无法进行调度配置和基线配置。

- 资源

​ 资源(resource)可被研发人员共享和利用的客观存在，如果想使用自定义函数则需要依赖资源。例如：编写 UDF 后，需要将编译好的 jar 包以资源的形式上传到 。运行此 UDF 时， 会自动下载这个 jar 包获取您的代码来运行 UDF，无需您干预。上传 jar 包的过程就是创建资源的过程，这个 jar 包是资源的一种类型。
...
函数使用系统的内建函数完成一定的计算和计数功能。但当内建函数无法满足要求时，您可以使用数栖平台提供的 java 编程接口开发自定义函数（User Defined Function，以下简称 UDF）。在开发完成 UDF 代码后，需要将代码编译成 jar 包，并将此 jar 包以 jar 资源的形式上传到，最后注册生成UDF。
...
参数(parameter)是使用通用变量来建立函数和变量之间关系。系统参数是指与系统状态、功能和行为等有关变量。
...
<img src="images/帐户开通a.png" style="zoom:100%"/>

​								<center>图2-1-1 帐户开通步骤a</center>

2.  进入信息填写界面，填写相应真实信息，点击“提交”。

<img src="images/帐户开通b.png" style="zoom:100%"/>

​								<center>图2-1-2 帐户开通步骤b</center>

3.  至此已完成申请流程，运营人员会在1-3天内尽快将相关帐户信息发送到您的邮箱；如果想要申请管理员帐户请联系在线客服人员。

#### 2.2 平台帐户开通

项目帐户指的是对单个空间项目下的具体功能点使用权限。

1. 用申请的帐户登陆数栖开发平台http://ide.dtwave-inc.com/，进入到“项目管理”界面。

<img src="images/项目帐户a.png" style="zoom:100%"/>

​								<center>图2-2-1 数栖开发平台项目管理界面</center>

2.  点击项目成员管理（只有平台管理员的权限才有此选项）此界面可查看已添加的成员，还能添加项目成员和移出项目成员；各成员角色请见“角色权限信息”附件。

<img src="images/项目帐户b.png" style="zoom:100%"/>

​								<center>图2-2-2 项目成员管理界面</center>

3.  点击“添加成员”，出现成员下拉列表（此处成员必须和申请的帐户为同一租户），可选多个成员，选择成员完成后，分配相应的角色；点击确定；出现 系统提示“操作成功”即添加成功。

<img src="images/添加成员a.png" style="zoom:100%"/>

​							     <center>图2-2-3 项目成员添加</center>

<img src="images/添加成员b.png" style="zoom:100%"/>

​							         <center>图2-2-4 项目成员添加</center>

4. 返回项目成员管理可看到刚两帐户已添加成功。

<img src="images/添加成员c.png" style="zoom:100%"/>

  <center>图2-2-5 项目成员列表</center>

5. 项目成员删除，选择成员后面的“操作”-“移出本项目”即可。
6. 项目成员批量删除，成员列表“方框”打勾选择，可多选；点击“批量删除”；即把成员删除。

### 3. 快速入门 

本文将演示如何将学生信息导入表中以及如何进行数据加工的流程。
...
​                                                             <center>图 3-1-1 Github上学生信息</center>
...
​                                                                <center>图 3-2-3 新建资源步骤a</center>

​	b. 输入资源名student_info，选择资源类型为txt，点击添加文件如图3-2-4所示：

​	文件类型支持文本txt、编译后的jar包、python脚本，且各文件名要以文件类型为后缀名。如本次要上传的文件**student_info.txt**以txt为后缀名。文件大小最多可上传50M。

<img src="images/上传资源d.png" style="zoom:100%" />

​                                                               <center>图 3-2-4 新建资源步骤b</center>

​	c. 选择创建好的student_info.txt文件，上传文件后会显示所上传资源信息。如图3-2-5所示，资源名为student_info.txt，大小为101B。点击确定按钮，完成资源上传，上传时间依文件大小而定。

<img src="images/上传资源e.png" style="zoom:100%" />

​                                                              <center>图 3-2-5 显示上传资源信息</center>

​	d. 其他操作：重命名文件、修改责任人和重新上传文件，可右击文件，选择**配置**进行更新，如图3-2-6所示：

<img src="images/上传资源f.png" style="zoom:100%"/>

​                                                             <center>图 3-2-6 选择配置,修改文件信息步骤</center>

​	在资源配置弹窗中修改文件名或修改责任人或重新上传文件。如图3-2-7所示：

<img src="images/上传资源g.png" style="zoom:100%"/>

​                                                            <center>图 3-2-7 选择配置,修改文件信息步骤</center>
...
​                                                              <center>图 3-3-1 任务目录</center>

2).  创建ddl与job目录

​	在新建的quick_start任务目录下，创建ddl与job目录，ddl目录存放建表任务、job任务存放数据处理任务。如图3-3-2所示：

<img src="images/新建表c.png" style="zoom:100%" />

​                                                                <center>图 3-3-2 创建ddl与job目录</center>

3).  在ddl目录下创建建表任务，右键单击ddl目录，在弹框中选择新建离线任务，如图3-3-3所示: 

<img src="images/新建表d.png" style="zoom:100%" />

​                                                                   <center>图 3-3-3 创建建表任务a</center>

4).  输入任务名ddl_quick_start_student_info，选择任务类型为DDL，点击确定，完成表创建工作，如图3-3-4所示。ddl目录下任务命名一般为ddl\_表名，此为推荐命名规则不强制要求，详细[命名规范](https://github.com/dtwave/shuxi/blob/master/doc/%E6%95%B0%E6%A0%96%E5%BC%80%E5%8F%91%E5%B9%B3%E5%8F%B0-%E5%B8%AE%E5%8A%A9%E6%96%87%E6%A1%A3.md)。任务类型为DDL，表明我们将采用DDL任务创建表，在任务中只用输入建表语句即可。

<img src="images/新建表e.png" style="zoom:100%" />

​                                                                  <center>图 3-3-4 创建建表任务b</center>
...
​                                                                <center>图 3-3-5 创建quick_start_student_info表</center>

运行部分日志如图3-3-6所示，如果显示**任务运行成功(Finished)**，则表示表创建成功。可以使用desc quick_start_student_info命令查看quick_start_student_info表字段信息，这里不再演示。

<img src="images/新建表g.png" style="zoom:100%"/>

​                                                                   <center>图 3-3-6 执行建表语句任务部分日志</center>

#### 3.4 导入数据

1)  创建导入数据任务

​	右键点击**job**目录，选择**新建离线任务**，任务为quick_start_student_info，任务类型为hive，点击**确定**，hive任务类型表明导入数据语句将以hive sql的方式运行。如图3-4-1、3-4-2所示：

<img src="images/导入数据c.png" style="zoom:100%"/>

​                                                                  <center>图 3-4-1 创建导入数据任务</center>

<img src="images/导入数据a.png" style="zoom:100%"/>

​                                                                    <center>图 3-4-2 创建导入数据任务</center>       
...
​                                                        <center>图 3-4-3 导入数据到quick_start_student_info表</center>

​	b. 点击**运行**，运行日志部分如图3-4-4所示，如果出现**任务运行成功(Finished)**，则表示导入数据成功。可以在日志中看到导入数据到表中以后，从表中查询出的数据，Hive任务查询出的结果不能保存，再次运行任务，上次任务结果将会被清空。

<img src="images/导入数据d.png" style="zoom:100%"/>

​                                                       <center>图 3-4-4 任务quick_start_student_info部分日志</center>
...
​                                                                  <center>图 3-5-1 创建数据加工任务步骤a</center>

<img src="images/加工数据a.png" style="zoom:100%"/>

​                                                                  <center>图 3-5-2 创建数据加工任务步骤b</center>
...
​                                                             <center>图 3-5-3 运行数据加工任务</center>

​	b. 点击运行，运行日志部分如图3-5-4所示，如果出现**任务运行成功(Finished)** 则表示语句执行成功。点击**运行结果1**、**运行结果2**，可以看到我们加工数据获得的结果。我们可以在任务执行过程中点击日志栏的垃圾箱图标来清空已有日志。

<img src="images/加工数据c.png" style="zoom:100%"/>

​                                                          <center>图 3-5-4 运行日志及结果</center>

​	sparksql获得的结果不会在下次执行任务的时候被清空。sparksql的执行结果最多返回5000条记录，可以通过翻页来查看数据，也可以把数据以csv格式下载下来。如图3-5-5所示：

<img src="images/加工数据d.png" style="zoom:100%"/>

​                                                          <center>图 3-5-5 运行日志及结果</center>
...
运行总览页可以查看离线任务、流式任务的统计信息，包含运行总任务数、当前时间任务运行数、已完成的任务数、待运行任务数、失败任务数等信息，点击任务数模块可以直接进入到实例详情页面，查看任务详情信息。

运行总览页离线任务信息任务如下图 4-3-1所示：

<img src="images/离线任务总览a.png" style="zoom:100%"/>

<center>图4-3-1 离线任务总览</center>

运行总览页流式任务信息如下图 4-3-2所示：

<img src="images/流式任务总览a.png" style="zoom:100%"/>

<center>图4-3-2 流式任务总览</center>
...
<img src="images/离线实例页a.png" style="zoom:100%"/>

<center>图4-3-3 离线实例页</center>
...
<img src="images/查看运行日志a.png" style="zoom:100%"/>

<center>图4-3-4 任务日志页</center>
...
<img src="images/查看代码a.png" style="zoom:100%"/>

<center>图4-3-5 任务代码页</center>
...
<img src="images/补数据a.png" style="zoom:100%"/>

<center>图4-3-6 补数据操作页</center>
...
<img src="images/流式任务a.png" style="zoom:100%"/>

<center>图4-3-7 流式任务页</center>

#### 4.4 监控管理

##### 4.4.1 基线管理

​	在基线中可以统一管理数据处理任务的预完成时间、数据处理任务的优先级、数据处理任务的告警策略、识别任务的基准开始和完成时间。通过基线可以监控任务的按时完成与否，当未按时完成时及时通知相关人员进行告警来处理。告警记录中保留了所有触发了告警的记录，相关责任人处理了告警的任务后，可以把状态设置为置为已处理。基线管理中可以新增基线、编辑基线、删除基线和基线列表查询等。

​	示例：任务的基线配置

<img src="images/基线管理.png" style="zoom:100%"/>

​                                                            <center>图4-4-1 任务的基线配置</center>

###### 4.4.1.1 新建基线

1）进入监控管理界面—基线管理界面，点击“新增基线”，进入到新增基线界面。

<img src="images/新增基线a.png" style="zoom:100%"/>

​                                                           <center>图4-4-2 新增基线入口</center>

2）填写以下相应的信息，“*”表示填项，下拉框如果不选，则为默认。

<img src="images/新增基线d.png" style="zoom:100%"/>

​                                                              <center>图4-4-3 新增基线</center>
...
<img src="images/编辑基线a.png" style="zoom:100%"/>

​                                                                    <center>图4-4-4 编辑基线</center>
...
<img src="images/告警处理a.png" style="zoom:100%"/>

​                                                                   <center>图4-4-5 基线告警记录处理</center>

2）批量“置为已处理”，把列表菜单栏中的告警ID前面的方框“勾上”，此时本页中的所有未处理的告警记录都为选中状态，点击列表上方的“置为已处理”按钮。

<img src="images/告警处理b.png" style="zoom:100%"/>

​                                                                <center>图4-4-5 基线告警记录批量处理</center>
...
<img src="images/数据质量告警页a.png" style="zoom:100%"/>

   <center>图4-4-6 数据质量页</center>
...
​	通过数据管理平台，可以实现数据的统一规范管理、企业数据类目管理，帮助企业来治理数据的缺、散、乱等问题。其中，元数据管理需要实现所有异构平台数据资源的统一元数据管理，为数据资源的统一调度奠定标准化基础，并通过元数据模块以历史事实的方式记录每项数据的来源，处理过程，应用对接情况等数据血缘追溯，同时实现数据资源存储的生命周期管理。
...
<img src="images/全局概览.png" style="zoom:100%"/>

​                                                                         <center>图4-5-1 全局概览</center>
...
<img src="images/元数据管理a.png" style="zoom:100%"/>

​                                                                 <center>图4-5-2 元数据管理界面</center>
...
<img src="images/元数据管理b.png" style="zoom:100%"/>

​                                                                <center>图4-5-3 元数据表详细信息</center>

- 明细信息：主要包括字段信息、分区信息、产出信息、数据预览等

  【字段信息】展示该表的数据字段和分区字段的名称、类型和描述信息。

  <img src="images/元数据管理c.png" style="zoom:100%"/>

  ​                                        	<center>图4-5-4 元数据表明细信息--字段信息</center>

  【分区信息】展示该表的分区信息，按照创建时间倒序展示表的所有分区信息

  <img src="images/元数据管理d.png" style="zoom:100%"/>

  ​						<center>图4-5-5 元数据表分区明细信息--分区信息</center>

  【产出信息】展现该表最近7个相关执行周期中新增加的记录条数，hover上去会自动显示记录数。

  <img src="images/元数据管理e.png" style="zoom:100%"/>

  ​						<center>图4-5-6 元数据表分区明细信息--产出信息</center>

  【数据预览】预览该表的10条记录

  <img src="images/元数据管理f.png" style="zoom:100%"/>

  ​						<center>图4-5-7 元数据表分区明细信息--数据预览</center>

###### 4.5.2.1 生命周期

​	数据表的生命周期指的是分区表数据的有效期；如A分区表生命周期为7天，指的是该表分区数据存储时间只有7天，7天过后会该分区数据被自动删除。在表的详细信息--存储信息中可设置生命周期：7天、30天、366天、永久，默认为永久。

1）元数据管理数据列表界面，点击表的详情进入到元数据表详细信息界面，点击存储信息中的生命周期的修改按钮。

<img src="images/生命周期a.png" style="zoom:100%"/>

​								<center>图4-5-8 元数据表生命周期设置1</center>

2）出现生命周期选择下拉框，选择需要的周期，点击“确定”按钮；出现系统提示“操作成功”，即设置成功。

<img src="images/生命周期b.png" style="zoom:100%"/>

​								<center>图4-5-8 元数据表生命周期设置2</center>

###### 4.5.2.2 数据血缘

​	数据血缘关系以历史事实的方式记录每项数据的来源，处理过程等，记录了数据表在治理过程中的全链血缘关系；基于这些血缘关系信息，可以轻松的进行数据分析，以数据流向为主线的血缘追溯等功能，从而提升报表信息的可信度。为数据的合规性提供验证手段，帮助业务部门实现信息共享、提升协调工作效率。通过元数据模块以历史事实的方式记录每项数据的来源，处理过程，应用对接情况等，记录了数据表在治理过程中的全链血缘关系，基于这些血缘关系信息，可以轻松的进行影响分析，以数据流向为主线的血缘追溯等功能；支持表级血缘分析、作业级血缘分析。
...
<img src="images/数据血缘信息a.png" style="zoom:100%"/>

​	<center>图4-5-9 表血缘信息</center>
...
​	术语英删除和修改和数据类目的删除及修改一致，请参照数据类目。

#### 4.6 项目管理
##### 4.6.1 项目配置

项目配置是对项目工作空间下基本信息、功能权限、任务类型等配置操作。功能权限包含：是否启用周期调度、是否允许直接编辑任务或代码、是否允许下载查询结果、下载结果限制、是否允许跨库访问等。 只有勾选了某任务类型，才可在开发中心新建该类型的任务。

项目配置界面如4-6-1图所示：

<img src="images/项目配置a.png" style="zoom:100%"/>

 <center>图 4-6-1 项目配置界面</center>

##### 4.6.2 项目成员管理

项目成员管理（只有平台管理员的权限才有此选项）此界面可查看已添加的成员，添加项目成员和移出项目成员；目前项目空间下有四个成员角色：开发、管理员、运维、访客，各成员角色请见“角色权限信息”附件。

项目成员管理界面如下所示：

<img src="images/项目帐户b.png" style="zoom:100%"/>

​								<center>图 4-6-2项目成员管理界面</center>

点击“添加成员”，出现成员下拉列表（此处成员必须和申请的帐户为同一租户），可选多个成员，选择成员完成后，分配相应的角色；点击确定；出现 系统提示“操作成功”即添加成功。

<img src="images/添加成员a.png" style="zoom:100%"/>

​							     <center>图4-6-3 项目成员添加</center>

添加完成后，在项目成员管理可看到刚添加的人员。

<img src="images/添加成员c.png" style="zoom:100%"/>

  <center>图4-6-4 项目成员添加完成</center>
...
<img src="images/资源组管理a.png" style="zoom:100%"/>

 <center>图4-6-5 资源组管理页面</center>

管理主机页如图4-6-6 所示：

<img src="images/资源组管理页b.png" style="zoom:100%"/>

 <center>图4-6-6 主机管理页面</center>
...
<img src="images/计算引擎管理页a.png" style="zoom:100%"/>

 <center>图4-6-7 计算引擎管理页面</center>
...
<img src="images/数据源管理页a.png" style="zoom:100%"/>

 <center>图4-6-8 数据源管理页面</center>

数据源新建如图4-6-9 所示，

<img src="images/数据源新建a.png" style="zoom:100%"/>

 <center>图4-6-9 数据源新建页面</center>
...
<img src="images/新建项目空间a.png" style="zoom:100%"/>

 <center>图4-7-7 项目空间添加导航页</center>

- 新建项目

  点击导航页“开始操作”，进入创建项目页面，填写项目名称、责任人、项目描述。创建项目页如图4-7-8 所示：

  <img src="images/创建项目a.png" style="zoom:100%"/>

   <center>图4-7-8 创建项目页</center>

- 添加资源组

  进入添加资源组页，填写资源组名称、添加主机列表和资源组描述。添加主机时，需要填写主机名称、ip地址(安装dubheNode插件的服务器地址)、任务运行最大并发数。点击确定会校验主机联通性。添加资源组界面如图4-7-9 所示：

  <img src="images/添加资源组a.png" style="zoom:100%"/>

   <center>图4-7-9 创建项目页</center>
...
- 配置计算引擎
  ...
  <img src="images/配置计算引擎a.png" style="zoom:100%"/>

   <center>图4-7-10 配置计算引擎</center>
...
​	dim层存放维表数据，本案例dim层存放业主所在小区和城市的映射关系数据。您需要先将维表数据上传到数栖开发平台，然后将数据导入表中。具体步骤如下:

1)  将把维表数据存储为txt格式的address_city.txt文件，且文件后缀名必须为'.txt'。

​	小区和城市的映射数据如下(字段之间以tab键分割):
...
4)  右键单击dim目录，选择**新建资源**，资源名address_city，资源类型txt，点击添加文件，选中要上传的address_city.txt文件，点击确定，完成上传。如图4-3-1-1所示：

<img src="images/dim_a.png" style="zoom:100%"/>

​                                                         <center>图 4-3-1-1 创建资源address_city</center>

5)  在开发中心下，导航至**离线任务**页面。  

6)  同样地，点击**离线任务目录**，在弹框中选择**新建目录**，创建demo目录，在demo目录下创建dim目录。

7)  在dim目录下新建ddl与job两个目录，ddl目录建表任务所在目录、job目录为数据处理任务目录。

8)  在demo/dim/ddl目录下创建任务。点击ddl目录，选中**新建离线任务**，任务名为`ddl_demo_dim_address_city`，任务类型为DDL，点击**确定**，然后在新建任务页面输入任务语句，点击**运行**，完成hive表创建工作。如图4-3-1-2所示：


<img src="images/dim_d.png" style="zoom:100%"/>

​                                                         <center>图 4-3-1-2 创建demo_dim_address_city表</center>
​	任务语句如下:
```sql
-- *********************************************************************
...
​	**注意**: 用户在数据开发中，不用指定数据库名。该任务在开发环境中执行的时候，表对应的就是开发环境的Hive库对应的表。发布到生产，在生产环境中执行的时候表对应的就是生产环境的Hive库对应的表。如果用户在表名前加了hive库名称，在开发和生产环境执行的时候，对应的表是用户指定的库中的表。开发环境和生产环境对应的库分别如图4-3-1-3、4-3-1-4所示:

<img src="images/dim_d_a.png" style="zoom:200%"/>

​                	 					<center>图 4-3-1-3 开发环境demo_dev下对应的库</center>

<img src="images/dim_d_b.png" style="zoom:200%"/>

​                 						<center>图 4-3-1-4 生产环境demo_prd下对应的库</center>

9)  在demo/dim/job目录下创建任务 

​	点击job目录，选中   **新建离线任务**，任务名为`demo_dim_address_city`，任务类型为SparkSQL，点击**确定**，然后点击界面右侧**属性配置**，在依赖配置里的资源依赖里选择上传的资源address_city.txt。在新建任务页面输入任务语句，点击**运行**，完成数据导入工作。如图4-3-1-5：<img src="images/dim_b.png" style="zoom:100%"/>

​                   					<center>图 4-3-1-5 导入资源数据到demo_dim_address_city表</center>
...
10)  运行部分日志如图4-3-1-6所示，在运行中如果返回日志太多，可以点击回收站图标来删除日志。

<img src="images/dim_c.png" style="zoom:100%"/>

​                 				<center>图 4-3-1-6 demo_dim_address_city任务运行部分日志</center>
...
​	3)  使用数栖平台的DataSync任务将mysql中demo_complaint_data与demo_users表中的数据同步到数栖平台hive库中。首先需要创建数据来源(数据从哪里来)，数据目的源(数据到哪里去)。

​	进入顶部菜单栏中的**项目管理**，导航至**数据源管理**页面，点击**新建数据源**。创建数据的来源`dtwave_demo_mysql`，以及数据的目的源`dtwave_demo_hive`两个数据源。分别如图4-3-2-1、4-3-2-2所示：

<img src="images/ods_a.png" style="zoom:100%"/>

​               	 					<center> 图 4-3-2-1 创建dtwave_demo_mysql数据源</center>

<img src="images/ods_b.png" style="zoom:100%"/>

​             	 					  <center>图 4-3-2-2 创建dtwave_demo_hive数据源</center>

​	4)  数据源创建完成以后，进行DataSync同步任务的开发。进入顶部菜单栏中的**开发中心**，导航至**离线任务**页面，在demo目录下创建ods目录，ods目录下新建ddl与job目录。

​	5)  在demo/ods/ddl目录下新建离线任务，任务名称:`ddl_demo_ods_complaint_data_d`，任务类型为DDL，点击**确定**，在新建任务页面输入任务语句，点击**运行**，完成hive表创建工作。任务语句如下:
...
​	6)  同样地在demo/ods/ddl目录下创建`ddl_demo_ods_users_info`任务，任务类型为DDL，按照上述操作创建demo_ods_users_info表。任务语句如下:
...
​	7)  将mysql库中的demo_complaint_data表中的数据同步到hive库demo_ods_complaint_data_d中。在demo/ods/job目录下新建DataSync任务，任务名称:`demo_ods_complaint_data_d`任务内容如下:  
...
​	8)  在demo/ods/job目录下新建DataSync任务，任务名称:'`demo_ods_users_info`'。此任务为同步任务，同步步骤2的mysql中'`demo_users`'表数据到IDE平台的`demo_ods_users_info` hive表中，具体步骤如下:
...
​	1.  编写udf函数，采用算法模型，从投诉内容中获取投诉区域、投诉对象、投诉问题。

​	2.  udf相关代码在[Github](https://github.com/dtwave/shuxi)上，clone下来在本地编译。

​	3. 在**开发中心**的**资源开发**下点击demo目录，在demo目录下新建dwd目录、dwd目录下新建udf目录。此目录下存放udf的jar包。(各目录名字依照各公司规范而定)

​	4. 点击步骤3创建的udf目录，选择新建资源。资源名:extract_complaint，资源类型:jar，然后在'选择文件'中添加文件，选择在步骤2编译好的udf函数jar包。

​	5. 在**开发中心**的**函数开发**下点击函数目录，新建demo目录，在demo目录下新建udf目录，点击udf目录选择新建函数来创建我们的udf函数，函数名称:extract_complaint、类名: com.dtwave.hive. ExtractComplaintUDF(类名全路径)、资源依赖: extract_complaint.jar，用途,命令格式,参数说明这三个参数并不强制要求填写。

  	小结：以上5步为创建udf函数的过程。在函数创建过程中使用的各种名称依各公司命名规范而定。

​	6. 在**开发中心**的**离线任务**的demo目录下新建dwd目录,同样在dwd目录下新建ddl与job两个目录。

​	7. 在demo/dwd/ddl目录下创建DDL任务，任务名为`ddl_demo_dwd_complaint_detail_d`，点击'确定'，然后在此任务下输入任务语句，点击'运行'，完成hive表创建工作。任务语句如下:
...
​	8. 在demo/dwd/ddl目录下创建DDL任务，任务名为`ddl_demo_dwd_users_and_complaint_detail_d`，点击'确定'，然后在此任务下输入任务语句，点击'运行'，完成hive表创建工作。任务语句如下:
...
​	9.  在demo/dwd/job目录下创建DDL任务，任务名为`demo_dwd_complaint_detail_d`，点击'确定'，然后在此任务下输入任务语句，点击'运行'，完成hive表创建工作。任务语句如下:

```sql
  -- *********************************************************************
  -- 功能：采用算法模型解析投诉内容,获取投诉位置,投诉对象,投诉问题！
  -- 时间：2018-02-02
  -- ********************************************************************
...
​	10.  在demo/dwd/job目录下创建DDL任务，任务名为`demo_dwd_users_and_complaint_detail_d`，然后在此任务下输入任务语句，点击'运行'，完成hive表创建工作。任务语句如下:
...
​	2.  在demo/tdm/ddl目录下创建DDL任务，任务名为`ddl_demo_tdm_city_history_object_count_d，点击'确定'，然后在此任务下输入任务语句，点击'运行'，完成hive表创建工作。任务语句如下:
...
​	3.  在demo/tdm/ddl目录下创建DDL任务，任务名为`ddl_demo_tdm_user_complaint_count_d`，点击'确定'，然后在此任务下输入任务语句，点击'运行'，完成hive表创建工作。任务语句如下:
...
​	4.  在demo/tdm/ddl目录下创建DDL任务，任务名为`ddl_demo_tdm_user_complaint_position_count_d`，点击'确定'，然后在此任务下输入任务语句，点击'运行'，完成hive表创建工作。任务语句如下:
...
​	5.  在demo/tdm/job目录下创建SparkSQL任务，任务名为`demo_tdm_city_history_object_count_d`，在此任务下输入任务语句，点击'运行'。任务语句如下:

```sql
  -- *********************************************************************
  -- 功能：统计每个城市历史以来的投诉对象次数
  -- 时间：2018-02-01
  -- *********************************************************************
...
​	6.  在demo/tdm/job目录下创建SparkSQL任务，任务名为`demo_tdm_user_complaint_count_d`，在此任务下输入任务语句，点击'运行'。任务语句如下:
...
​	1.  在**开发中心**的**离线任务**的demo目录下新建adm目录，同样在adm目录下新建ddl与job两个目录。

​	2.  在demo/adm/ddl目录下创建DDL任务，任务名为`ddl_demo_adm_history_complaint_d`，在此任务下输入任务语句。任务语句如下:
...
​	3.  在demo/adm/job目录下创建SparkSQL任务，任务名为`demo_adm_history_complaint_d，在此任务下输入任务语句。任务语句如下:

```sql
  -- *********************************************************************
  -- 功能：历史以来的各个业主投诉位置与投诉对象的次数统计
  -- 时间：2018-02-01
  -- ********************************************************************
...
1.把我们在上面开发的任务配上调度时间,基线,资源依赖,上游任务,参数等.点击各个任务页面上的'提交'按钮,填写好提交信息(非必需),然后点击'确定',提交任务!
​ 
2.在**发布中心**下选中我们刚才提交的任务,这时在'右侧的待发布对象'中会看到我们选中的任务,然后点击'创建发布包',填写发布包名称:,发布描述:'',点击确定完成创建发布包.

3.点击'发布中心'下的发布历史,可以看到我们刚才创建的发布包.这时通知有发布权限的管理员发布我们的发布包!

  注意:发布任务前一定要在测试环境做好测试!
