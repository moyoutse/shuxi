...
任务（Task）描述数栖开发平台数据采集和处理过程的基本单元。具体任务类型请详见本文**3.1.3 任务类型**。任务可自由配置参数、调度时间、任务间的依赖关系和任务基线等；多个任务间可配置依赖关系，组成DAG图。
...
资源(Resource)可被研发人员共享和使用的客观存在。目前支持的三种资源: jar、txt、python。
...
这些学生信息已经上传到Github，[student_info.txt](https://github.com/dtwave/shuxi/blob/master/data/student/student_info.txt)如图2-1-1所示：

<img src="images/student_info.png" style="zoom:100%"/><center>图 2-1-1 Github上学生信息</center>
...
​ 进入数栖开发平台后，在**开发中心**下的**资源开发**页面，右键点击**资源目录**，选择**新建目录**。如图2-2-1所示：

<img src="images/上传资源a.png" style="zoom:100%"/> <center>图 2-2-1 新建资源目录</center>

目录名为**quick_start**，点击**确定**完成目录创建，资源文件student_info.txt将放在此目录下。如图2-2-2所示：

<img src="images/上传资源b.png" style="zoom:100%" /><center>图 2-2-2 新建资源目录</center>

2)  新建资源

右键点击上步创建的**quick_start**目录，选择新建资源。如图2-2-3所示：

<img src="images/上传资源c.png" style="zoom:100%" /><center>图 2-2-3 新建资源步骤</center>

输入资源名student_info，选择资源类型为txt，点击添加文件如图2-2-4所示。例如：本次要上传的文件**student_info.txt**以txt为后缀名。最多可上传50M的文件。

<img src="images/上传资源d.png" style="zoom:100%" /><center>图 2-2-4 新建资源步骤</center>

选择创建好的student_info.txt文件，上传文件后会显示所上传资源信息。如图2-2-5所示，资源名为student_info.txt，大小为101B。

<img src="images/上传资源e.png" style="zoom:100%" /><center>图 2-2-5 显示上传资源信息</center>

其他操作：重命名文件、修改责任人和重新上传文件，可右击文件，选择**配置**进行更新，如图2-2-6所示：

<img src="images/上传资源f.png" style="zoom:100%"/>  <center>图 2-2-6 修改资源信息</center>

在资源配置弹窗中修改文件名或修改责任人或重新上传文件。如图2-2-7所示：

<img src="images/上传资源g.png" style="zoom:100%"/><center>图 2-2-7 修改资源信息</center>
...
在**开发中心**下，点击**离线任务**，右键点击**离线任务目录**，选择**新建目录**，新建quick_start目录。如图2-3-1所示：

<img src="images/新建表a.png" style="zoom:100%" />           <center>图 2-3-1 任务目录</center>

2)  创建ddl与job目录

在新建的quick_start任务目录下，创建ddl与job目录。如图2-3-2所示：

<img src="images/新建表c.png" style="zoom:100%" /><center>图 2-3-2 创建ddl与job目录</center>

3)  在ddl目录下创建建表任务，右键单击ddl目录，在弹框中选择新建离线任务，如图2-3-3所示: 

<img src="images/新建表d.png" style="zoom:100%" /><center>图 2-3-3 创建DDL任务</center>

4)  输入任务名ddl_quick_start_student_info，选择任务类型为DDL，如图2-3-4所示。
...
5)  点击运行，创建quick_start_student_info表。如图2-3-5所示：

<img src="images/新建表f.png" style="zoom:100%"/><center>图 2-3-5 创建quick_start_student_info表</center>

运行部分日志如图2-3-6所示，如果显示**任务运行成功(Finished)**，则表示表创建成功，也可以使用desc quick_start_student_info命令查看表字段信息查看是否创建成功。
...
右键点击**job**目录，选择**新建离线任务**，任务名为quick_start_student_info，任务类型为hive。如图2-4-1、2-4-2所示：

<img src="images/导入数据c_mg.png" style="zoom:100%"/><center>图 2-4-1 创建导入数据任务</center>

<img src="images/导入数据a.png" style="zoom:100%"/><center>图 2-4-2 创建导入数据任务</center>       
...
任务需要在页面右侧的属性配置里设置资源依赖，资源为我们前面上传的**student_info.txt**文件，点击**运行**，执行数据导入语句。如图2-4-3所示：

<img src="images/导入数据b.png" style="zoom:100%"/> <center>图 2-4-3 数据导入</center>

​ 运行日志部分如图3-4-4所示，如果出现**任务运行成功(Finished)**，则表示导入数据成功。也可以通过select 语句从表中查询数据，Hive任务查询出的结果不能保存，再次运行任务，上次任务结果将会被清空。

<img src="images/导入数据d.png" style="zoom:100%"/><center>图 2-4-4 任务运行部分日志</center>
...
右键点击**job**目录，选择**新建离线任务**，任务为quick_start_student_statistics，任务类型为SparkSQL，点击**确定**。如图2-5-1、2-5-2所示：

<img src="images/导入数据c_mg.png" style="zoom:100%"/>  <center>图 2-5-1 创建数据加工任务</center>

<img src="images/加工数据a.png" style="zoom:100%"/> <center>图 2-5-2 创建数据加工任务</center>
...
运行进行数据加工，获得学生基本信息及学生最大年龄和最小体重。如图2-5-3所示:

<img src="images/加工数据b.png" style="zoom:100%"/><center>图 2-5-3 运行数据加工任务</center>

点击**运行结果1**、**运行结果2**，可以看到我们加工数据获得的结果，我们可以在任务执行过程中点击日志栏的垃圾箱图标来清空已有日志。

<img src="images/加工数据c.png" style="zoom:100%"/><center>图 2-5-4 运行日志</center>

SparkSQL任务获得的结果不会在下次执行任务的时候被清空，并且执行结果最多返回5000条记录，可以通过翻页来查看数据，也可以把数据以csv格式下载下来。如图2-5-5所示：

<img src="images/加工数据d.png" style="zoom:100%"/><center>图 2-5-5 运行结果</center>
...
创建离线任务，选择任务将要存放目录，右击目录，选择**新建离线任务**，自定义任务名称，选择任务类型，点击**确定**，完成任务创建。如图3-1-1-1、3-1-1-2所示：

<img src="images/导入数据c_mg.png" style="zoom:100%"/><center>图 3-1-1-1 创建离线任务</center>

<img src="images/加工数据a_mg.png" style="zoom:100%"/><center>图 3-1-1-2 创建离线任务</center>

###### 3.1.1.2 复制

复制离线任务，新任务名默认为原文件名_copy，如图3-1-1-3、 3-1-1-4所示:

<img src="images/copy_a_mg.png" style="zoom:100%"/><center>图 3-1-1-3 复制离线任务</center>

<img src="images/copy_b_mg.png" style="zoom:100%"/><center>图 3-1-1-4 复制离线任务</center>

###### 3.1.1.3 删除

只能删除开发环境离线任务，右击要删除的任务，选择**删除离线任务**，点击**确定**，完成删除。如图3-1-1-5、3-1-1-6 所示 。删除需慎重，删除后不可恢复，需要删除生产环境离线任务时，先在开发环境删除该任务，通过发布中心重新发布进行删除。

<img src="images/delete_a_mg.png" style="zoom:100%"/><center>图 3-1-1-5 删除离线任务</center>

<img src="images/delete_b_mg.png" style="zoom:100%"/><center>图 3-1-1-6 删除离线任务</center>

###### 3.1.1.4 格式化

格式化语句，选中想要格式化的语句，点击**格式化**。如图3-1-1-7所示：

<img src="images/format_a_mg.png" style="zoom:100%"/><center>图 3-1-1-7 格式化语句</center>

###### 3.1.1.5 代码检查

选中需要检查语句，点击**代码检查**，进行语法校验，目前只支持sql语法校验，如图3-1-1-8所示：

<img src="images/check_a_mg.png" style="zoom:100%"/><center>图 3-1-1-8 代码检查</center>

###### 3.1.1.6 运行

直接点击运行按钮即会运行任务所有代码。当需要运行某一语句时，选中要执行的语句，再点击**运行**即可。如图3-1-1-9所示：

<img src="images/eva_a_mg.png" style="zoom:100%"/> <center>图 3-1-1-9 运行</center>

###### 3.1.1.7 提交

任务在开发环境完成测试以后，可以发布到生产环境，点击提交后在**发布中心**的**创建发布包**页面会有一条记录待发布。如图3-1-1-10 所示：

<img src="images/submit_a_mg.png" style="zoom:100%"/><center>图 3-1-1-10 提交任务</center>
...
- 用户自定义参数
  ...
  参数左侧是key或变量名，右侧是value或变量值，使用时在语句中是${变量名}，如图3-1-2-1所示：
...
- 正常调度
  ...
  <img src="images/正常调度_mg.png" style="zoom:100%"/><center>图 3-1-2-2 正常调度</center>

- 跨周期调度

  如果周期设置为每周，可以选择每周的具体某天运行任务；如果选择每月，可以理选择每月的具体某一号运行任务。如图3-1-2-3 所示：

  <img src="images/跨周期调度_mg.png" style="zoom:100%"/><center>图 3-1-2-3 跨周期调度</center>

- 暂停调度（白松）

  暂停调度即线上环境任务不触发周期调度。配置如图3-1-2-4 所示：
...
资源依赖、任务依赖配置如图3-1-2-5 所示：

<img src="images/依赖配置_mg.png" style="zoom:100%"/><center>图 3-1-2-5 依赖配置</center>

###### 3.1.2.4 基线配置

通过配置基线监控任务截止到具体某个时刻是否还在运行，对仍在运行的任务进行告警。配置如图3-1-2-6 所示：

<img src="images/基线配置_mg.png" style="zoom:100%"/><center>图 3-1-2-6 基线配置</center>

###### 3.1.2.5 资源组配置

任务可以配置到某个资源组中运行，配置如图3-1-2-7 所示：

<img src="images/资源组配置.png" style="zoom:100%"/><center>图 3-1-2-7 资源组配置</center>
...
<img src="images/DataSync_a_mg.png" style="zoom:100%"/><center>图 3-1-3-2 DataSync任务</center>

###### 3.1.3.3 Hive

<img src="images/hive_a_mg.png" style="zoom:100%"/><center>图 3-1-3-3 Hive任务</center>

###### 3.1.3.4 SparkSQL

<img src="images/sparksql_a_mg.png" style="zoom:100%"/><center>图 3-1-3-4 SparkSql任务</center>

###### 3.1.3.5 Python

<img src="images/python_a_mg.png" style="zoom:100%"/><center>图 3-1-3-5 Python任务</center>

###### 3.1.3.6 PySpark

<img src="images/pyspark_a_mg.png" style="zoom:100%"/><center>图 3-1-3-6 PySpark任务</center>

###### 3.1.3.7 Spark

<img src="images/spark_a_mg.png" style="zoom:100%"/><center>图 3-1-3-7 Spark任务</center>

###### 3.1.3.8 Hive2

<img src="images/hive2_a_mg.png" style="zoom:100%"/><center>图 3-1-3-8 Hive2任务</center>

###### 3.1.3.9 Presto

<img src="images/presto_a_mg.png" style="zoom:100%"/><center>图 3-1-3-9 Presto任务</center>
